...
-(void)hideSelf:(MBProgressHUD *)hud{
    
    hud.removeFromSuperViewOnHide = YES;
    [hud hide:YES];
}
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view info:(NSString *)info animated:(BOOL)animated hideTimes:(int )time{
	MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.detailsLabelText = info;
	[view addSubview:hud];
	[hud show:animated];
    [hud performSelector:@selector(hideSelf:) withObject:hud afterDelay:time];
	return MB_AUTORELEASE(hud);
}
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view info:(NSString *)info animated:(BOOL)animated {
	MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.labelText = info;
	[view addSubview:hud];
	[hud show:animated];
